...
const getRestaurantDetails = (payload) => {
    return dispatch => {
        return axios.get(`http://localhost:3001/restaurant/detail/${payload.restaurant_id}`)
        .then(response => {
            if(response.status === 200) {
                dispatch({ type: actionTypes.SET_CUSTOMER_SELECTED_RESTAURANT, payload: response.data});
            }
        })
    }
}
...
const placeOrder = (payload, ownProps) => {
    return dispatch => {
        ...
        return axios.post('http://localhost:3001/order/confirm', payload)
        .then(response => {
            if(response.status === 200) {
